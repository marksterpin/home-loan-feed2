    ...
    while (i < mortgages.length) {
      ...
      try {
        ...
        result.products.push({
          id: p.productId,
          lender: (p.brand || p.brandName || lender.name),   // per-product brand: splits shared group endpoints (Westpac group serves St.George, Bank of Melbourne, BankSA from one URL) instead of collapsing them under one register name
          name: p.name || "",
          ...
          fees: (d.fees || []).slice(0, 20).map(f => ({
            name: f.name || f.feeType || "Fee",
            amount: f.amount != null ? num(f.amount) : null,
            balanceRate: f.balanceRate != null ? num(f.balanceRate) : null,   // % of balance p.a.
            transactionRate: f.transactionRate != null ? num(f.transactionRate) : null,
            feeType: (f.feeType || "").toUpperCase(),                          // PERIODIC / UPFRONT / EXIT / EVENT / TRANSACTION / WITHDRAWAL / DEPOSIT
            ...
            discounts: (f.discounts || []).slice(0, 6).map(x => ({
              ...
              amount: x.amount != null ? num(x.amount) : null,
              balanceRate: x.balanceRate != null ? num(x.balanceRate) : null,
              info: (x.additionalInfo || "").slice(0, 100),
...
async function main(){
  ...
  async function lenderWorker(){
    while (q < lenders.length) {
      ...
      lenderMeta.push({ name: r.name, base: r.base, status: r.status, error: r.error || null, productCount: r.productCount, listed: r.listed != null ? r.listed : null, eligible: r.eligible != null ? r.eligible : null, detailFailed: r.detailFailed || 0, detailError: r.detailError || null, emptyReason: r.emptyReason || null });
      ...
      else console.log(`  ✓ ${r.name} — ${r.productCount} mortgages${r.detailFailed ? " (" + r.detailFailed + " detail call(s) failed)" : ""}`);
    }
  }
  ...
  await writeFile(`${OUT_DIR}/meta.json`, JSON.stringify({
    ...
    lenders: out.lenders.map(l => ({ name: l.name, status: l.status, error: l.error, productCount: l.productCount, listed: l.listed, eligible: l.eligible, detailFailed: l.detailFailed, detailError: l.detailError, emptyReason: l.emptyReason }))
  }, null, 2));
