...
async function harvestLender(lender){
    ...
    while (i < mortgages.length) {
      ...
      try {
        ...
        result.products.push({
          id: p.productId,
          lender: lender.name, _brand: ((p.brand || p.brandName || "") + "").trim(),   // capture raw product brand; resolved endpoint-aware below
          name: p.name || "",
     ...
     per-product codes (e.g. TMB, AMB, C, E) some holders put in the product `brand` field. */
  {
    ...
    for (const x of result.products) { x.lender = (_multi && x._brand) ? x._brand : lender.name; delete x._brand; }
  }
